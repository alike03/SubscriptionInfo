...
var allowedTags = ["h2", "h3", "h4", "div", "span", "a", "picture", "source", "img", "ul", "li", "p", "i", "b", "br", "select", "option", "optgroup", "input", "label"];
var allowedAttributes = ["id", "class", "style", "href", "srcset", "loading", "title", "alt", "target", "value", "type", "name", "for", "placeholder", "checked"];
    ...
    if (allowedTags.includes(content.element)) {
      ...
      if (content.hasOwnProperty('attributes')) {
        for (var attr in content.attributes) {
          ...
          if (allowedAttributes.includes(attr) || attr === 'src' && content.element === 'img' || attr.startsWith('data-')) {
...
function loadOptions() {
  ...
  fetch(page).then(function (response) {
    ...
    var showNoInfoBar = document.querySelector('#aSubO_showNoInfoBar');
    showNoInfoBar.checked = save.options.showNoInfoBar;
    showNoInfoBar.addEventListener('change', function (e) {
